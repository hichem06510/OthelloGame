...
public class JavaFXBoardGameApplicationLauncher implements BoardGameApplicationLauncher {
    ...
    public void launchApplication(BoardGameConfiguration configuration,
        ...
        if (Platform.isFxApplicationThread()) {
            // Si nous sommes déjà sur le thread JavaFX, démarrez directement l'application
            Platform.runLater(() -> {
                try {
                    new JavaFXBoardGameApplication().start(new Stage());
                } catch (Exception e) {
                    e.printStackTrace();
                }
            });
        } else {
            // Sinon, utilisez Application.launch() pour démarrer JavaFX
